...
void CUDAGenerator(uint64_t chainLen, uint64_t chainCount, const char *suffix, const char *type)
{
    ...
    sprintf(fileName,"%s_%lld-%lld_%s-cuda", type, (long long)chainLen, (long long)chainCount, suffix);
    ...
    for(int round = 0; round < time1; round++)
    {
        ...
        for(uint64_t i = 0; i < ALL; i++)
        {
            ...
            if(i_type == 0)
            {
                starts[i] &= totalSpace_Global_DES;
                starts[i] = Convert(starts[i] + i, 6) & totalSpace_Global_DES;
            }
        }

...
{
    Logo();
    printf("Usage:     hmaccuda chainLen chainCount suffix\n\n");

    printf("example 1: hmaccuda 1000 10000 suffix\n");
}

__global__ void HMACGeneratorCUDA(uint64_t *data)
{
    __syncthreads();
    ...
    for(int nPos = 0; nPos < CHAINLEN; nPos++)
        value = Cipher2MSG_HMAC(MSG2Ciper_HMAC(value), nPos);
...
void HMACGenerator(uint64_t chainLen, uint64_t chainCount, const char *suffix)
{
    char fileName[100];
    sprintf(fileName,"HMAC_%lld-%lld_%s-cuda", (long long)chainLen, (long long)chainCount, suffix);
    ...
    for(int round = 0; round < time_0; round++)
    {
        ...
        HMACGeneratorCUDA<<<BLOCK_LENGTH, MAX_THREAD>>>(cudaIn);
...
int main(int argc, char * argv[])
{
    ...
    HMACGenerator(chainLen, chainCount, suffix);

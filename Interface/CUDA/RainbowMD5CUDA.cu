...
{
    Logo();
    printf("Usage:     md5cuda chainLen chainCount suffix\n\n");

    printf("example 1: md5cuda 1000 10000 suffix\n");
}

__global__ void MD5GeneratorCUDA(uint64_t *data)
{
    __syncthreads();
...
void MD5Generator(uint64_t chainLen, uint64_t chainCount, const char *suffix)
{
    char fileName[100];
    sprintf(fileName,"MD5_%lld-%lld_%s-cuda", (long long)chainLen, (long long)chainCount, suffix);
    ...
    for(int round = 0; round < time_0; round++)
    {
        ...
        MD5GeneratorCUDA<<<BLOCK_LENGTH, MAX_THREAD>>>(cudaIn);
...
int main(int argc, char * argv[])
{
    ...
    MD5Generator(chainLen, chainCount, suffix);

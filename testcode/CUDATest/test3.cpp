...
#define LONGCAT(hi,low,bit) ((((unsigned long)hi)<<(bit/2))+(unsigned)low)

unsigned long des_applypc(unsigned long key,unsigned long *pc,int len)
{
    unsigned long ret=0;
    int i=0;
    ret=(key&pc[0])!=0?1:0;
...
/**
    ...
    14974415778239806716 = 0xcfcfcfcffcfcfcfc;
**/
unsigned long des_lrol(int *hi,int *low,int loop)
{
    while(loop>0)
    {
        *hi = LCROL(*hi,28);
    ...
    cout<<(*hi)<<" "<<(*low)<<endl;
    unsigned long ret = LONGCAT(*hi,*low,56);
    return ret;
}

void des_ckey(unsigned long key,unsigned long *store)//生成轮换key
{   
    unsigned long tmp;
...
int main()
{
	unsigned long key = 0xFEFEFEFEFEFEFEFE,store[16];
    des_ckey(key,store);
	FILE*file=fopen("Txt.txt","wb");
	assert(file);
	assert(fwrite((char*)&(store[0]),sizeof(unsigned long),16,file)==16);
    for(int i=0;i<16;i++)
    {
        cout<<store[i]<<endl;
    }

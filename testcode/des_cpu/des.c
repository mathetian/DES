...
#define LONGCAT(hi,low,bit) (((int64)hi<<(bit/2))+(unsigned)low)

int64 des_applypc(int64 key,int64 *pc,int len)
{
    int64 ret;int i;
    ret=0;i=0;
    ret=(key&pc[0])!=0?1:0;
...
int64 des_lrol(int *hi,int *low,int loop)
{
    while(loop>0)
    {
        *hi = LCROL(*hi,28);
    ...
    int64 ret = LONGCAT(*hi,*low,56);
    return ret;
}
void des_ckey(int64 key,int64 *store)//生成轮换key
{
    int64 tmp;
...
int64 des_one(int64 data,int64 key)
{
    int hi=HIGH(data);
    int low=LOW(data);
    int64 tmp;
...
int64 des(int64 data,int64 *ckey,int index)
{
    int i,j,d;
    int64 t;
...
{
    struct timeval tstart, tend;int numproc,rank;
    MPI_Init(&argc,&argv);
    MPI_Comm_size(MPI_COMM_WORLD, &numproc);
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    gettimeofday(&tstart, NULL);
    srand(time(0));int64 data, key;
    for(int i=0;i<(1<<15);i++)
    {
	   data=rand(), key=rand();
	   int64 ckey[16];
       des_ckey(key,ckey);
       data=des(data,ckey,i);
    }
    printf("%lld\n",data);
    gettimeofday(&tend, NULL);
    printf("loop time: %d s, %d us\n",(int)(tend.tv_sec - tstart.tv_sec),(int)(tend.tv_usec - tstart.tv_usec));
    MPI_Finalize(); 
   return 0;
}
